...
//echo $action.'**';
		...
		if($model == 'admin')
			$this->load("admin-header");
		else if(!is_mobile())
			$this->load("desktop-header");
        else $this->load("header");
        ...
        if($model != 'admin' && is_mobile())
           $this->load("footer");
		else if($model != 'admin' && !is_mobile())
			$this->load("footer-desktop");
    ...
    /**
     * Used to assign the page description of the rendered HTML file.
     *
     * @param   $description      Description of the rendered HTML file
     */
